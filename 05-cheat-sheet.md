...
```
Policy     → "The rules" (individual conditions, 7 effects including Deny and Modify)
Initiative → "A bundle of policies" (grouped toward a goal)
Blueprint  → "The whole package" (policies + ARM + RBAC + RGs) ⚠️ RETIRING → Template Specs + Deployment Stacks
```
...
- **TCO Calculator = DEPRECATED** (August 2025). Pricing Calculator is now the tool for ALL cost estimation including migration scenarios.
- **Data is always YOUR responsibility** across IaaS, PaaS, AND SaaS — you own governance, classification, and access control.
- **Composite SLA** = multiply individual SLAs (e.g., 99.95% × 99.99% = 99.94%). More services = lower composite SLA.
